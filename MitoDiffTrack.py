# ============================================
# Script name: 20251127_mit_tracker_plus.py
#
# Purpose:
#   - Detect and track mitochondria in time-lapse movies
#   - CellProfiler-like IdentifyPrimaryObjects style detection
#   - Global LAP tracking for all mitochondria
#   - Flow-motion filament map:
#       * Dense optical flow between frames
#       * Accumulate long-term motion score per pixel
#       * Threshold and skeletonize to get motion_filament_skel
#       * Ignore intensity: dark but fast mito still contribute
#   - NEW: Fast-track pipeline inside skeleton ROI:
#       * Dilate motion_filament_skel to ROI
#       * Aggressive detection inside ROI only
#       * Separate LAP tracking for fast tracks
#       * Preview layer mito_fast_tracks_preview
#   - Export:
#       * Global per-object detection table (all frames)
#       * Global per-track summary (LAP) with speed_um_per_s
#       * Global per-step table (LAP) with speed_um_per_s
#       * Optional fast-track per-track and per-step CSV (skeleton ROI)
#       * Optional snapshot PNG of the viewer
#   - GUI config:
#       * Save current GUI parameters to JSON
#       * Load JSON and restore GUI parameters with widget values shown
#
# Author: Mo
# Last modified: 2025-11-27 12:45
# ============================================
...
# ============================================
# Preprocess and identify primary objects
# ============================================
...
def identify_preview(viewer: napari.Viewer,
                     ...
                     close_radius: int = 0):
    """
    Preview segmentation at current time frame.
    """
...
# ============================================
# Global acquisition parameters dock
# ============================================
...
def acquisition_params(viewer: napari.Viewer,
    ...
    """
    Global acquisition parameters. Tracking will use these values.
    """
    global LAST_META
...
def detect_all_frames(viewer: napari.Viewer,
                      ...
                      close_radius: int = 0):
    """
    Run intensity-based object detection on all frames in the chosen time range.
    """
...
# ============================================
# Stage 2 - LAP tracking (global)  [unchanged]
# ============================================
...
def lap_tracking(viewer: napari.Viewer,
    ...
    """
    Run LAP tracking on intensity-based detections and build preview.
    """
...
# ============================================
# Stage 3 - Flow motion filaments (skeleton)
# ============================================
...
def _flow_motion_filament_core(data: np.ndarray,
    ...
    """
    Flow-based filament map (more central skeleton):

      - 对 t ∈ [t0, t1-1] 每对相邻帧算 dense optical flow
      - 累加 motion magnitude 得到 motion_accum
      - 强平滑 + 降百分位，把“高速走廊”整体填满
      - 再 skeletonize 得到更靠走廊中轴的 skeleton
    """
    if data.ndim == 3:
        T, Y, X = data.shape
...
def flow_motion_filaments(viewer: napari.Viewer,
                          ...
                          min_area_px: int = 10):
    """
    Build motion-based filament skeleton from long-term optical flow pattern.

    Controls:
      - thr_percentile: how aggressive to keep only top-speed highways
      - downscale: speed vs detail trade-off
    Output layer:
...
def fast_tracks_skeleton(viewer: napari.Viewer,
    ...
    """
    Fast-track pipeline inside skeleton ROI:

      - Requires motion_filament_skel generated by Flow motion filaments
      - Builds dilation ROI around skeleton
      - Runs aggressive detection only inside ROI
      - Then runs LAP tracking with larger max disp
      - Outputs mito_fast_tracks_preview and FAST_TRACK_STEPS_DF with speed_um_per_s
    """
    ...
    if pixel_size_um is None or frame_dt_s is None or frame_dt_s <= 0:
        show_error("Pixel size or frame interval not set. Please set them in 'Pixel / Time' dock first.")
        FAST_TRACK_STEPS_DF = None
        return
...
# ============================================
# Export dock (global LAP + optional fast tracks)
# ============================================
...
def export_tracks(viewer: napari.Viewer,
    ...
    """
    Export detection, global LAP tracks, optional fast tracks, per-step tables, and snapshot PNG.

    Export is only allowed if pixel_size_um and frame_dt_s are set,
    so that all track files include speed_um_per_s.
    """
    ...
    if pixel_size_um is None or frame_dt_s is None or frame_dt_s <= 0:
        show_error(
            ...
            "Please set them in 'Pixel / Time' dock and click Apply."
        )
        return
...
def _collect_gui_params() -> Dict:
    ...
    meta = {
        "version": "mit_tracker_plus_flowfilament_fastROI",
...
def main():
    ...
    viewer.window.add_dock_widget(load_czi,              name="📂 Load CZI",                        area="right")
    viewer.window.add_dock_widget(acquisition_params,    name="⏱ Pixel / Time",                     area="right")
    viewer.window.add_dock_widget(identify_preview,      name="🔍 Segmentation preview",            area="right")
    viewer.window.add_dock_widget(detect_all_frames,     name="① Detection (all frames)",           area="right")
    viewer.window.add_dock_widget(lap_tracking,          name="② LAP tracking (global)",            area="right")
    viewer.window.add_dock_widget(flow_motion_filaments, name="③ Flow motion filaments",            area="right")
    viewer.window.add_dock_widget(fast_tracks_skeleton,  name="③b Fast tracks (skeleton ROI)",      area="right")
    viewer.window.add_dock_widget(export_tracks,         name="④ Export CSV / PNG",                 area="right")
    viewer.window.add_dock_widget(save_gui_config,       name="💾 Save GUI config",                 area="right")
    viewer.window.add_dock_widget(load_gui_config,       name="📂 Load GUI config",                 area="right")

    _center_and_resize(viewer)
    show_info(
        "Workflow:\n"
        "1) 📂 Load CZI\n"
        "2) ⏱ Set pixel size and dt (for µm/s)\n"
        "3) 🔍 Segmentation preview (optional)\n"
        "4) ① Detection (all frames, intensity-based)\n"
        "5) ② LAP tracking (global object tracks with µm/s)\n"
        "6) ③ Flow motion filaments (long-term motion skeleton, ignore intensity)\n"
        "7) ③b Fast tracks (aggressive detection only inside skeleton ROI)\n"
        "8) ④ Export (per-object + LAP + FAST steps) with speed_um_per_s\n"
        "9) 💾 Save / 📂 Load GUI config\n"
    )
    napari.run()
